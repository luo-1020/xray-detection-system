...
from PyQt5.QtWidgets import QDialog, QMessageBox, QFileDialog, QApplication  # Add QApplication import
from PyQt5.QtCore import QTimer  # Add this import for QTimer
...
class MyForm(QDialog):
    ...
    def show_video(self):
        ...
        if flag:
            ...
            if self.video_count is not None:
                self.ui.textBrowser.append(f'{self.print_id}/{self.video_count} Frames. time:{end-since:.5f}s ' + self.analyse_result(result))
            else:
                self.ui.textBrowser.append(f'{self.print_id} Frames. time:{end-since:.5f}s ' + self.analyse_result(result))
            self.print_id += 1
        else:
            self.now = None
            self.reset_timer()
